...
namespace TimePlayer.UserControls.Behaviour
{
	public static class Masking
	{
		private static readonly DependencyPropertyKey
			_maskExpressionPropertyKey = DependencyProperty.RegisterAttachedReadOnly("MaskExpression", typeof(Regex), typeof(Masking), new FrameworkPropertyMetadata());
		...
		private static void textBox_PreviewKeyDown(object sender, KeyEventArgs e)
		{
			...
			// Same story with Delete
			else if (e.Key == Key.Delete)
			{
				proposedText = GetProposedTextDelete(textBox);
			}

			if (proposedText != null && !IsValid(proposedText))
			{
				e.Handled = true;
			}
		}
		...
		private static bool IsValid(string text)
		{
			...
			return (IsTimeSpanValid(parts[0]) && IsTimeSpanValid(parts[1]));
		}

		private static bool IsTimeSpanValid(string str)
		{
			string[] parts = str.Split(':');
			if (parts.Length != 2) return false;

			return (IsNumValid(parts[0]) && IsNumValid(parts[1]));
		}

		private static bool IsNumValid(string str)
		{
